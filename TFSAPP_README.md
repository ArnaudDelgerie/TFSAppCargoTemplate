...
> 📄 **Ce fichier est le README de référence du wrapper TFS app**, posé par le template à la
> génération. Il décrit comment développer, builder et packager l'app produite. Renomme-le en
> `README.md` ou reprends-en ce dont tu as besoin dans ton propre README orienté métier.

Application desktop Tauri 2 qui embarque une application Symfony servie localement par FrankenPHP —
une **Tauri FrankenPHP Symfony application** (« TFS app »), générée depuis le template
`TFSAppCargoTemplate`.
...
Ce n'est pas un POC jetable : c'est une **base de départ pour de vraies apps locales**. Le contrat
précis entre le launcher et l'app est décrit dans [CONTRACT.md](CONTRACT.md).

> Identité de cette app : `productName` (`<ProductName>`), `identifier` (`<identifier>`) et le binaire
> (`<project-name>`) sont définis dans `desktop/src-tauri/tauri.conf.json` et
> `desktop/src-tauri/Cargo.toml`. Ces valeurs pilotent le nom du `.deb`
> (`<ProductName>_<version>_amd64.deb`), le chemin `/usr/lib/<ProductName>/` et l'app-data
> `~/.local/share/<identifier>`. Tout le reste en **dérive automatiquement** (le code Rust est
> découplé du nom). ⚠️ **Ne jamais changer l'`identifier` après publication** → perte de l'app-data
> utilisateur (voir [Adapter cette app](#adapter-cette-app) et [CONTRACT.md §10](CONTRACT.md)).
...
> Cette section décrit l'app de base livrée par le template. Si tu as remplacé le contenu de `app/`
...
## Activer / désactiver l'async

Le traitement asynchrone (worker Messenger) est piloté par une **seule constante Rust**, fixée à la
génération depuis la réponse `with_async` :
...
- lance `messenger:consume async` et **le supervise** (relance automatique à chaque sortie), si l'async est activé ;
...
## Mise à jour du wrapper depuis le template

À la génération, un fichier **`.scaffold.toml`** a été écrit à la racine du projet : il enregistre la
version du template d'origine (`template_version`) et les réponses utilisées. Ne pas l'éditer à la
main — il sert à deux choses :

- **Savoir si tu es en retard** : comparer `template_version` avec la dernière version publiée du
  template `TFSAppCargoTemplate` (son `CHANGELOG.md`). Un bump `MAJOR` signale un changement
  *wrapper-breaking* dont les étapes de migration sont décrites dans le changelog du template.
- **Mettre à jour le wrapper** : comme le wrapper (`desktop/`, `docker/`, `build/`, `compose.yaml`,
  `Makefile`, Caddyfiles) est orthogonal à `app/`, la mise à jour consiste à réécrire cette couche
  **sans toucher `app/` ni l'identité**. En attendant un script dédié : `git diff` entre deux tags du
  template sur ces chemins, appliqué à la main.

⚠️ Ne **jamais** changer l'`identifier` après publication (perte de l'app-data utilisateur — voir
...
- **Écrire dans les ressources installées.** `/usr/lib/<ProductName>/resources/app` appartient au
...
(En prod, pas de `-vvv` : les logs passent par le logger Symfony sous `APP_LOG_DIR`. Le worker
n'apparaît que si l'async est activé.)
...
L'identité a été fixée à la génération. Pour la modifier ensuite (le code Rust est **découplé du nom**
— titre de fenêtre lu depuis la config, chemins de cleanup dérivés au runtime, logs génériques) :
...
`msi`…). Le contrat complet est dans [CONTRACT.md](CONTRACT.md).
